...
func TestPHPExtractVariableRoutingAndConfiguration(t *testing.T) {
	for _, mode := range []string{"enabled", "domain-off", "framework"} {
		t.Run(mode, func(t *testing.T) {
			...
			notifyAdministrationDocumentOpen(t, client, uri, 1, "<?php\nreturn build();\n")
			waitForAdministrationDiagnostics(t, published, uri)
			params := map[string]any{"textDocument": map[string]any{"uri": uri}, "range": protocol.Range{Start: protocol.Position{Line: 1, Character: 8}, End: protocol.Position{Line: 1, Character: 8}}, "context": map[string]any{"only": []string{"refactor.extract"}, "diagnostics": []protocol.Diagnostic{}}}
			...
			require.NoError(t, client.Notify(context.Background(), "textDocument/didChange", map[string]any{"textDocument": map[string]any{"uri": uri, "version": 2}, "contentChanges": []map[string]any{{"text": "<?php\nreturn 42;\n$extracted = 1;\n"}}}))
			waitForAdministrationDiagnostics(t, published, uri)
			actions = nil
			require.NoError(t, client.Call(context.Background(), "textDocument/codeAction", params, &actions))
			require.Len(t, actions, 1)
			assert.Equal(t, "Extract variable '$extracted2'", actions[0].Title)
