...
func (s *Server) initialize(ctx context.Context, params *protocol.InitializeParams) interface{} {
	...
	return map[string]interface{}{
		"capabilities": map[string]interface{}{
			...
			// Note: shopware.twig.extendBlock / focusExtendedBlock are intentionally
			// NOT advertised in executeCommandProvider. The VS Code extension registers
			// them as client-side commands (custom UI); advertising them would make the
			// language client register duplicates and fail ("command already exists").
			// Editors without the extension (e.g. Zed) still invoke them as code-action
			// commands, which the workspace/executeCommand handler below serves.
